...
/*   Updated: 2024/06/04 08:56:41 by ymomen           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	start_simulation(t_data *data)
{
	...
	else
	{
		...
		{
			if (pthread_create(&data->philo[i].philo, NULL, simulation_routine,
					&data->philo[i]))
				return (error("Error: thread create\n"));
		}
		monitor(data, 0, 0);
		i = -1;
		while (++i < data->nb_philo)
